...
export const Form = <TOptions extends Record<string, any>, TResult = any>({
      ...
      if (inputs?.[key]) {
        ...
        if (input.type === 'select') {
          if (input && 'options' in input) {
            options[key] =
              (input.options?.find((opt) => typeof opt === 'object' && opt.value === options[key]) as { value: any })
                ?.value || options[key]
          }
        }
      }
  ...
  return (
    <form {...props} className="flex flex-col gap-2" onSubmit={handleSubmit}>
      <div className="space-y-1">
        <div className="flex gap-2">
          {fnName && <h2 className="text-md font-semibold">{fnName}</h2>}
      ...
      <div className={cn('flex flex-col gap-2 w-full', props.className)}>
              ...
              <div key={name} className="flex gap-2 items-center">
                {name && typeof name === 'string' && (
                  <div className="flex gap-0">
                    <div className="text-xs text-red-500 transform -translate-y-1">{input.required ? '*' : ''}</div>
                    <VariableTooltip name={name} variable={input} actualType={getValueType(input.type)}>
                      {input.required ? (
        ...
        {resultError && (
          <div className="mt-2 p-2 bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 rounded">
            ...
            <BaseVariable name="return" value={functionResult} depth={1} maxDepth={10} defaultExpanded={1} />
          </div>
        )}
