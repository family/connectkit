...
const formatValue = (val: any, valueType: string, name: string) => {
  switch (valueType) {
    case 'string':
      return `"${val}"`;
    case 'number':
...
const EditableVariable = ({
  variable, value, onChange, actualType = 'string', name
}: {
  variable: HookInput
  value: any;
  ...
  return (
    <div className='group/edit relative w-full'>
      ...
      >
        ...
        <PenLine size={12} className='opacity-50 group-hover:opacity-70 group-hover/edit:opacity-100 transition-all duration-300' />
        <span
          // Underline
          className={cn(
      ...
      </button>
      {
        isEditing && (
          <div className={cn(
            actualType === 'select' ?
              "-left-3" : "left-0",
            'absolute -top-2 max-w-full w-lg z-10'
          )}>
            <InputAny
...
export const BaseVariable = ({
  ...
  const isExpandable = (actualType === 'object' || actualType === 'array' || (actualType === 'function' && hasVariable)) &&
    value !== null &&
    depth < maxDepth;
